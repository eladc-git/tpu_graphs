...
_RUN_ID = flags.DEFINE_string('run_id', '', 'Can be used for tagging the experiment.')
_VER = flags.DEFINE_string('ver', 'vX.X.X', 'Version')
_SEED = flags.DEFINE_integer('seed', 0, 'Seed')
_LAST = flags.DEFINE_bool('last', False, 'K fold')
...
def _get_results_csv_or_default() -> str:
  ...
  """
    ...
    if not results_csv:
        results_csv = os.path.join(os.path.expanduser(os.path.join(_OUTPUT_DIR.value,_VER.value)),
            f'results_{_VER.value}_tile.csv')
...
def get_args() -> TrainArgs:
    return TrainArgs(
        ...
        batch_size=_BATCH.value, configs=_NUM_CONFIGS.value, ver=_VER.value, seed=_SEED.value, last=_LAST.value,
