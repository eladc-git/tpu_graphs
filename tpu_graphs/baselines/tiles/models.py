...
class _OpEmbedding(tf.keras.Model):
    ...
    def __init__(self, num_ops: int, embed_d: int, l2reg: float = 1e-4):
        super().__init__()
        self.embedding_layer = tf.keras.layers.Embedding(num_ops, embed_d, activity_regularizer=tf.keras.regularizers.l2(l2reg))
...
class _SAGE(tf.keras.Model, _ConfigFeatureJoiner):
    ...
    def forward(self, graph: tfgnn.GraphTensor, num_configs: int) -> tfgnn.GraphTensor:
        ...
        bidirectional_adj = implicit.Sum(bidirectional_adj, bidirectional_adj.transpose())
...
class _EarlyJoin(_ConfigFeatureJoiner):
    ...
    def get_op_node_features(
        ...
        return tf.concat([
            ...
            graph.node_sets['op']['config_feats'], # add scale factor
