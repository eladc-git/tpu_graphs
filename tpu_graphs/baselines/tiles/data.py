...
class NpzDatasetPartition:
    ...
    def adapt_node_features(node_feat):
        return node_feat[:, [0, 1, 2, 6, 7, 8, 9, 10, 14, 15]]
    ...
    """
        ...
        assert npz_data['config_feat'].shape[1] == 24
        #npz_data['config_feat'] = self.adapt_node_features(npz_data['config_feat'])
    ...
    def get_graph_tensors_dataset(self, config_samples: int = -1) -> tf.data.Dataset:
        ...
        dataset = dataset.map(functools.partial(TileExample.to_graph_tensor, config_samples=config_samples))
        return dataset
...
def get_npz_split(
    ...
    if cache_dir:
        ...
        filename_hash = hashlib.md5(f'{split_path}:{min_configs}:{_TOY_DATA.value}'.encode()).hexdigest()
...
def get_npz_dataset(
        root_path: str, min_train_configs=-1, train="train", valid="valid",
  ...
  """
    npz_dataset = NpzDataset(
        train=get_npz_split(os.path.join(root_path, train), min_configs=min_train_configs,cache_dir=cache_dir),
