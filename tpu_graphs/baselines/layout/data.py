...
class LayoutExample(NamedTuple):
    ...
    def to_graph_tensor(self, config_samples: int = -1, max_nodes: int = -1) -> tfgnn.GraphTensor:
    ...
    """
        ...
        if max_nodes >= 0:
            ...
            segment_id = tf.random.uniform(shape=[], minval=0, maxval=num_segments, dtype=tf.int32)
            start_idx = segment_id * max_nodes
            end_idx = (segment_id + 1) * max_nodes
            end_idx = tf.minimum(end_idx, self.total_nodes)
            node_is_selected = tf.logical_and(op_node_ids >= start_idx, op_node_ids < end_idx)

            feed_edge_mask = tf.logical_and(self.edges >= start_idx, self.edges < end_idx)
            feed_edge_mask = tf.logical_and(feed_edge_mask[:, 0], feed_edge_mask[:, 1])
            config_edge_mask = tf.logical_and(full_config_tgt >= start_idx, full_config_tgt < end_idx)

            kept_node_ratio = tf.cast((end_idx - start_idx) / self.total_nodes, tf.float32)
...
class NpzDatasetPartition:
    ...
    @staticmethod
    def adapt_node_config_feat(config_feat):
        return config_feat[:,:,[0,1,2,6,7,8,12,13,14]]

    @staticmethod
    def adapt_node_features(node_feat):
        return node_feat[:,[0,*list(range(4,18)),21,22,23,31,32,33,37,38,39,43,44,*list(range(109,125)),134,135,136]]

    def add_npz_file(
            self, graph_id: str, npz_file: np.lib.npyio.NpzFile,
            min_configs: int = 2, max_configs=-1, rep=0):
    ...
    """
        ...
        #npz_data['node_config_feat'] = self.adapt_node_config_feat(npz_data['node_config_feat'])
        #npz_data['node_feat'] = self.adapt_node_features(npz_data['node_feat'])
        npz_data['node_splits'] = npz_data['node_splits'].reshape([-1])
        npz_data['argsort_config_runtime'] = np.argsort(npz_data['config_runtime'])
        if 0 < max_configs < num_configs:
            if rep==0:
                third = max_configs // 3
                keep_indices = np.concatenate([
                    npz_data['argsort_config_runtime'][:third],  # Good configs.
                    npz_data['argsort_config_runtime'][-third:],  # Bad configs.
                    np.random.choice(npz_data['argsort_config_runtime'][third:-third],max_configs - 2 * third, replace=False)
                ], axis=0)
                # -> random -----------------------------------------
                # keep_indices = np.random.choice(npz_data['argsort_config_runtime'], max_configs, replace=False)
                # -> random without edges ----------------------------
                # keep_indices = np.random.choice(npz_data['argsort_config_runtime'][third:-third], max_configs - 2 * third)
                # -------------------------------------------------
            else:
                keep_indices = np.random.choice(npz_data['argsort_config_runtime'], max_configs, replace=False)
        ...
        npz_data['node_config_feat'] = npz_data['node_config_feat'].reshape((num_configs * num_config_nodes, -1))
    ...
    def finalize(self):
        ...
        Afterward, caller is able to call `get_item()` on this class instance."""
        ...
        self.node_config_feat = tf.concat(self._data_dict.pop('node_config_feat'), axis=0)
        self.config_runtime = tf.concat(self._data_dict.pop('config_runtime'), axis=0)
        self.argsort_config_runtime = tf.concat(self._data_dict.pop('argsort_config_runtime'), axis=0)
    ...
    def get_item(self, index: int) -> LayoutExample:
        ...
        config_tensor = tf.reshape(flat_config, [num_configs, num_config_nodes, flat_config.shape[-1]])
        ...
        return LayoutExample(
            ...
            node_config_ids=tf.cast(self.node_config_ids[config_node_start:config_node_end], tf.int32),
            node_splits=self.node_splits[node_split_start:node_split_end],
            config_runtimes=self.config_runtime[config_start:config_end],
            argsort_config_runtimes=(self.argsort_config_runtime[config_start:config_end]),
    ...
    def get_graph_tensors_dataset(self, config_samples: int, max_nodes: int = -1) -> tf.data.Dataset:
        ...
        dataset = dataset.map(functools.partial(LayoutExample.to_graph_tensor, config_samples=config_samples, max_nodes=max_nodes))
        return dataset
    ...
    @property
    def num_ops(self):
        return int(tf.reduce_max(self.node_opcode).numpy()) + 1
    ...
    def normalize(self):
        normalizer_args = self._get_normalizer(self.node_feat)
        self.node_feat = self._apply_normalizer(self.node_feat, *normalizer_args)

        normalizer_args = self._get_normalizer(self.node_config_feat)
        self.node_config_feat = self._apply_normalizer(self.node_config_feat, *normalizer_args)
...
class NpzDataset(NamedTuple):
    ...
    """
        ...
        self.train.node_feat = self._apply_normalizer(self.train.node_feat, *normalizer_args)
        self.validation.node_feat = self._apply_normalizer(self.validation.node_feat, *normalizer_args)
        self.test.node_feat = self._apply_normalizer(self.test.node_feat, *normalizer_args)

        normalizer_args = self._get_normalizer(self.train.node_config_feat)
        self.train.node_config_feat = self._apply_normalizer(self.train.node_config_feat, *normalizer_args)
        self.validation.node_config_feat = self._apply_normalizer(self.validation.node_config_feat, *normalizer_args)
        self.test.node_config_feat = self._apply_normalizer(self.test.node_config_feat, *normalizer_args)


def get_npz_split(
        split_path: str, min_configs=2, max_configs=-1, rep=0,
    ...
    if cache_filename and tf.io.gfile.exists(cache_filename):
        ...
        for filename in tqdm.tqdm(files):
            ...
            npz_dataset.add_npz_file(graph_id, np_data, min_configs=min_configs, max_configs=max_configs ,rep=rep)
...
def get_npz_dataset(
        root_path: str, min_train_configs=-1, max_train_configs=-1, rep=0,
  ...
  """
    npz_dataset = NpzDataset(
        train=get_npz_split(
            os.path.join(root_path, 'train'), cache_dir=cache_dir, rep=rep,
...
def get_npz_valid_dataset(
        root_path: str, min_train_configs=-1, max_train_configs=-1,
        cache_dir: 'None | str' = None) -> NpzDatasetPartition:

    npz_valid = get_npz_split(os.path.join(root_path, 'valid'),
                              cache_dir=cache_dir,
