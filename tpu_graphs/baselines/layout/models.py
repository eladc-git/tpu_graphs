...
class ResModel(tf.keras.Model):
    ...
    def _node_level_forward(
        ...
        config_features = 100 * (adj_config @ config_features) # scale x100 the config features
    ...
    def forward(self, graph: tfgnn.GraphTensor, num_configs: int, backprop=True) -> tf.Tensor:
        ...
        node_features = tf.concat([graph.node_sets['op']['feats'], graph.node_sets['op']['op_e']], axis=-1)
